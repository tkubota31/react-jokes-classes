  ...
  render() {
    ...
    return (
      <div className="JokeList">
        <button className="JokeList-getmore" disabled={allLocked}onClick={this.generateNewJokes}>
          Get New Jokes
          </button>
          <button onClick= {this.resetVotes}>Reset</button>
        {sortedJokes.map(j => (
          <Joke text={j.joke} key={j.id} id={j.id} votes={j.votes} vote={this.vote} locked={j.locked} toggleLock={this.toggleLock} />
        ))}
